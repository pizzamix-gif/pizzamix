...
def payment_method_kb() -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="💵 Наличными", callback_data="pay:cash")
    b.button(text="💳 Терминал (карта)", callback_data="pay:terminal")
    b.button(text="📲 Переводом", callback_data="pay:transfer")
    b.adjust(1)
    return b.as_markup()


def payment_proof_kb(order_id: int) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
...
def admin_settings_kb(settings) -> InlineKeyboardMarkup:
    ...
    b.button(text="💳 Реквизиты (перевод)", callback_data="adm:set:transfer_details")
...
def admin_payment_confirm_kb(order_id: int) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="✅ Подтвердить оплату", callback_data=f"payconfirm:{order_id}")
    b.button(text="❌ Отклонить", callback_data=f"paydecline:{order_id}")
    b.adjust(1)
    return b.as_markup()
