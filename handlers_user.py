...
from utils import PAYMENT_METHOD_LABELS, fmt_dt, fmt_price
...
async def _ask_payment_method(message: Message, state: FSMContext, edit: bool):
    await state.set_state(None)
    text = "💳 Способ оплаты:"
    markup = kb.payment_method_kb()
    if edit:
        await message.edit_text(text, reply_markup=markup)
    else:
        await message.answer(text, reply_markup=markup)
...
async def _show_summary(message: Message, pool, state: FSMContext, edit: bool):
    ...
    lines.append(f"Оплата: {PAYMENT_METHOD_LABELS.get(payment_method, payment_method)}")
...
async def skip_comment(callback: CallbackQuery, state: FSMContext):
    ...
    await _ask_payment_method(callback.message, state, edit=True)
    await callback.answer()
...
async def got_comment(message: Message, state: FSMContext):
    ...
    await _ask_payment_method(message, state, edit=False)


@router.callback_query(F.data.startswith("pay:"))
async def choose_payment_method(callback: CallbackQuery, pool, state: FSMContext):
...
async def _notify_admins_text(bot, text: str, reply_markup=None):
...
async def _order_summary_text(pool, order_id: int, header: str) -> str:
    order = await database.get_order_with_user(pool, order_id)
    items = await database.get_order_items(pool, order_id)
    who = order["first_name"] or order["username"] or order["telegram_id"]
    lines = [
        header,
        f"От: {who}",
...
async def order_confirm(callback: CallbackQuery, pool, state: FSMContext, bot):
    ...
    delivery_price = float(settings["delivery_price"]) if delivery_type == "delivery" else 0.0
    payment_method = data.get("payment_method", "cash")
    payment_status = "pending" if payment_method == "transfer" else "confirmed"
    ...
    if payment_method == "transfer":
        # Order exists, but nobody in the kitchen is told yet — the admin
        # notification only goes out once payment proof shows up. That's
        # the actual spam filter. We don't rely on FSM state to know this
        # order is awaiting proof — a DB lookup below handles that even if
        # the customer wanders off and sends the screenshot much later.
        requisites = settings["transfer_details"] or "⚠️ Реквизиты ещё не настроены — свяжитесь с администратором."
        await state.set_data({})
        await state.set_state(None)
        await callback.message.edit_text(
            f"✅ Заказ №{order['id']} создан!\n"
            f"Сумма к переводу: {fmt_price(order['total_with_delivery'])}₽\n\n"
            f"Переведите на:\n{requisites}\n\n"
            f"После оплаты пришлите скриншот перевода или нажмите кнопку ниже.",
            reply_markup=kb.payment_proof_kb(order["id"]),
        )
        await callback.answer()
        return

    # Cash / terminal — unchanged: admin is told immediately, order starts right away.
    await state.set_data({})
    await state.set_state(None)
    ...
    await callback.message.edit_text(
        ...
        f"Оплата: {PAYMENT_METHOD_LABELS.get(payment_method, payment_method)}\n"
        f"Сумма к оплате: {fmt_price(order['total_with_delivery'])}₽\n\n"
        f"Мы свяжемся с вами в ближайшее время. Спасибо за заказ! 🍕"
    )
    await callback.answer()

    admin_text = await _order_summary_text(pool, order["id"], f"🆕 Новый заказ №{order['id']}")
    admin_text += f"\nОплата: {PAYMENT_METHOD_LABELS.get(payment_method, payment_method)}"
    await _notify_admins_text(bot, admin_text)


# ============================================================================
# Payment proof for "перевод" orders — screenshot or the "Я оплатил(а)"
# button. Either way, the admin gets ONE notification with confirm/decline
# buttons; nothing happens automatically past this point.
# ============================================================================

@router.message(F.photo)
async def got_customer_photo(message: Message, pool, bot):
    user = await database.get_user_by_telegram_id(pool, message.from_user.id)
    order = await database.get_pending_transfer_order(pool, user["id"]) if user else None
    if not order:
        await message.answer(
            "Если это скриншот оплаты — не нашёл заказ, который его ждёт. "
            "Если недавно оформляли заказ переводом, напишите нам напрямую — разберёмся."
        )
        return

    order_id = order["id"]
    file_id = message.photo[-1].file_id
    await database.set_order_payment_proof(pool, order_id, file_id)
    caption = await _order_summary_text(pool, order_id, f"💰 Скриншот оплаты — заказ №{order_id}")

    for admin_id in config.ADMIN_IDS:
        try:
            await bot.send_photo(
                admin_id, file_id, caption=caption, reply_markup=kb.admin_payment_confirm_kb(order_id)
            )
        except Exception:
            logger.exception("Failed to notify admin %s about payment proof for order %s", admin_id, order_id)

    await message.answer("✅ Спасибо! Отправили администратору на проверку — ждите подтверждения.")


@router.callback_query(F.data.startswith("paid:"))
async def payment_confirmed_by_button(callback: CallbackQuery, pool, state: FSMContext, bot):
    order_id = int(callback.data.split(":", 1)[1])
    order = await database.get_order_with_user(pool, order_id)
    if order["payment_status"] != "pending":
        await callback.answer("Этот заказ уже обработан, спасибо!", show_alert=True)
        return

    text = await _order_summary_text(pool, order_id, f"💰 Оплата заявлена (без скриншота) — заказ №{order_id}")
    await _notify_admins_text(bot, text, reply_markup=kb.admin_payment_confirm_kb(order_id))

    await callback.message.edit_text("✅ Спасибо! Отправили администратору на проверку — ждите подтверждения.")
    await callback.answer()
    await state.clear()
