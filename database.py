...
# Runs after SCHEMA on every startup. ALTER ... ADD COLUMN IF NOT EXISTS is
# idempotent, so this is safe to re-run — it's what actually lets us evolve
# the orders/settings tables on a database that was already deployed once.
MIGRATIONS = """
...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_proof_file_id TEXT;
ALTER TABLE settings ADD COLUMN IF NOT EXISTS transfer_details TEXT NOT NULL DEFAULT '';
"""
...
async def find_product(pool, category: str, name: str):
    async with pool.acquire() as conn:
        return await conn.fetchrow(
            "SELECT * FROM products WHERE category = $1 AND name = $2", category, name
        )


async def update_product(pool, product_id: int, description: str, weight: str, price: float):
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE products SET description = $1, weight = $2, price = $3 WHERE id = $4",
            description, weight, price, product_id,
        )
            ...
            order = await conn.fetchrow(
                ...
                INSERT INTO orders
                    (user_id, total_price, delivery_price, total_with_delivery,
                     delivery_type, address, phone, comment, payment_method, payment_status)
...
async def get_order_with_user(pool, order_id: int):
    async with pool.acquire() as conn:
        return await conn.fetchrow(
            """
...
async def get_pending_transfer_order(pool, user_id: int):
    async with pool.acquire() as conn:
        return await conn.fetchrow(
            """
            SELECT * FROM orders
            WHERE user_id = $1 AND payment_method = 'transfer' AND payment_status = 'pending'
...
async def update_order_payment_status(pool, order_id: int, status: str):
    async with pool.acquire() as conn:
...
async def set_order_payment_proof(pool, order_id: int, file_id: str):
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE orders SET payment_proof_file_id = $1 WHERE id = $2", file_id, order_id
        )
