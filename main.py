...
async def self_ping_loop() -> None:
    ...
    Same loop also double-checks the webhook is still registered — Render
    briefly overlaps old/new instances on deploy, and if an old instance's
    shutdown ever races a new one's startup, this restores it within one
    interval instead of the bot silently going deaf until the next deploy."""
        ...
        while True:
            ...
            try:
                ...
                if info.url != config.WEBHOOK_URL:
                    ...
                    logger.warning("Webhook was missing/wrong (was %r) — restored.", info.url)
            except Exception:
                logger.warning("Webhook watchdog check failed", exc_info=True)
...
async def on_shutdown(bot: Bot) -> None:
    if pool is not None:
        await pool.close()
    # Deliberately NOT calling bot.delete_webhook() here. Render briefly runs
    # the old and new instance side by side during a deploy — the new one
    # sets the webhook on startup, and if the old one then deletes it on its
    # own shutdown, the bot ends up with no webhook at all after every deploy.
